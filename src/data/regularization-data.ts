/**
 * regularization-data.ts — Topic 23 data module (split from regression-data.ts
 * per PR #27 review).
 *
 * Surface (brief §7.E):
 ...
 *
 * §7.E.4 (reuse of EXAMPLE_10_GLM_DATA for §23.6 Ex 8) requires no new data —
 * the existing Topic-22 export is consumed verbatim from regression-data.ts.
 *
 ...
 * fetched once at authoring time and embedded as TS literals so the site is
 * fully offline-capable.
 *
 * Why this lives in its own file (not regression-data.ts): the IIFE-computed
 * paths/CV at the bottom run at module load (~50–150 ms for the prostate data
 * with n=97 / p=8 / 100-pt λ-grid). regression-data.ts is imported by the 8
 * Topic 21 / Topic 22 visualization components that don't need any of this —
 * keeping Topic-23 data here means those unrelated bundles don't pay the cost
 * (PR #27 review feedback). Only RegularizationPathExplorer,
 * CrossValidationExplorer, and regression.test.ts import from this module.
 */
...
import { EXAMPLE_6_GLM_DATA } from './regression-data';
...
// Cost: ~50–150 ms compute per JS bundle execution (SSG: once at build;
// client: once per browser hydration of any component that imports this
// module). Confined to this file (separated from regression-data.ts per
// PR #27) so the 8 Topic-21/22 components that don't need this don't pay
// the cost. Future precomputation-to-static-JSON via a build script is
// deferred tech debt; IIFE is acceptable for the prostate problem size.
 ...
 *
 * All entries use family='gaussian' (the only family `crossValidate`
 * currently supports — see the family-guard added in PR #27).
 */
