...
A **multiple-testing procedure** is a function $(p_1, \ldots, p_m) \mapsto R \subseteq \{1, \ldots, m\}$ returning the set of **rejected** hypotheses. Counts of interest: $R = |R|$ (total rejections), $V = |R \cap H_0|$ (false rejections), $S = |R \cap H_0^c|$ (true rejections), $R = V + S$.
</DefinitionBlock>
...
For a multiple-testing procedure producing rejection set $R$ on data with true-null configuration $H_0$, the **family-wise error rate at $H_0$** is

$$\text{FWER}(H_0) \;=\; \Pr_{H_0}(V \ge 1) \;=\; \Pr_{H_0}(R \cap H_0 \ne \emptyset).$$
...
<RemarkBlock number={8} title="Hom on: Bonferroni as a level-$\\alpha$ test of the global null">
...
For a multiple-testing procedure with rejection set $R$ of size $R$ and false-rejection count $V$, the **false discovery proportion** is
...
**Step 3 — Apply the BEN2001 Lemma 2.1 independence lemma.** Under independence, the conditional distribution of $k^*$ given $p_i \le k \alpha/m$ — restricted to the event that BH stops at a rank $k$ such that null $i$ is included in the rejection set — is the same as the distribution of the step-up procedure applied to the *other* $p$-values at a modified threshold. Concretely (<ExternalLink href="https://doi.org/10.1214/aos/1013699998">BEN2001</ExternalLink> Lemma 2.1):

$$\mathbb{E}_{H_0}\!\left[\frac{\mathbf{1}\{p_i \le k^* \alpha / m\}}{R \vee 1}\right] \;=\; \frac{\alpha/m}{1} \cdot \Pr_{H_0}\!\left(p_i \le \tfrac{k^* \alpha}{m}\right) \cdot \frac{1}{\alpha/m} \;=\; \frac{\alpha}{m}.$$

The key cancellation is that the increment $\alpha/m$ in threshold per rank matches the increment $1/(R \vee 1)$ in denominator per rejection — this is exactly why the BH threshold is *linear* in $k$ rather than Bonferroni's constant $\alpha/m$. The lemma is where independence enters.

**Step 4 — Sum over the $m_0$ true nulls.** Each of the $m_0$ per-null contributions equals $\alpha/m$, so

$$\mathrm{FDR} \;=\; \sum_{i \in H_0} \frac{\alpha}{m} \;=\; \frac{m_0 \cdot \alpha}{m} \;=\; \pi_0 \cdot \alpha \;\le\; \alpha. \quad\blacksquare$$

∎ — using linearity of expectation (Step 1–2) and the BEN2001 Lemma 2.1 independence identity (Step 3); BEN1995 original; BEN2001 for the modern cleaner argument.
</ProofBlock>
...
These are Topic 20's immediate neighbors and are covered in depth in <ExternalLink href="/topics/hypothesis-testing">Track 6</ExternalLink>'s regression material.
</RemarkBlock>
...
- **Track 6 (Regression)** uses Bonferroni-adjusted coefficient CIs as the default simultaneous-inference tool and BH for variable selection. F-tests for nested-model comparisons are the closed-testing ancestor of Holm (§20.5 Rem 11).
