...
formalcalculusPrereqs:
  - topic: "power-taylor-series"
    site: "formalcalculus"
    relationship: "The log-partition function's moment-generating properties use Taylor-expansion arguments. The relationship between $A(\\eta)$ and the moment-generating function connects to power series — $A'(\\eta) = \\mathbb E[T(X)]$ and $A''(\\eta) = \\mathrm{Var}(T(X))$ are the first two Taylor coefficients."
  - topic: "mean-value-taylor"
    site: "formalcalculus"
    relationship: "The cumulant generating function $K(t) = \\log M_X(t)$ has Taylor coefficients equal to the cumulants $\\kappa_n$: $K(t) = \\sum \\kappa_n t^n/n!$. The exponential-family structure makes these derivatives tractable in closed form, and the mean-value-theorem remainder bound governs higher-order corrections."
formalmlConnections:
  ...
  - topic: "mixed-effects"
    site: "formalml"
    relationship: "Every GLM requires an exponential family response distribution and the canonical link is (A')⁻¹. Logistic, Poisson, and Gamma regression are developed in §6.2 of the Mixed-Effects Models topic as the GLMM extension to non-Gaussian likelihoods; the random-effects framework subsumes fixed-effects GLMs as the no-random-effects special case."
...
In each case, the canonical link emerges directly from the exponential family structure — it is the function $\eta(\mu) = (A')^{-1}(\mu)$ that maps from the mean parameterization to the natural parameterization. [Generalized Linear Models](/topics/generalized-linear-models#section-22-3) develops the full theory, including non-canonical links, iteratively reweighted least squares, and deviance analysis.
</ExampleBlock>
...
- [Generalized Linear Models](/topics/generalized-linear-models) develops the complete GLM framework — estimation via IRLS, deviance, model diagnostics — requiring the exponential family response distribution established here
