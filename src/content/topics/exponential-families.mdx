...
The seven distributions that are *not* exponential family members — Hypergeometric, Discrete Uniform, Continuous Uniform, Chi-squared (as a standalone), Student's $t$, and $F$ — fail for specific structural reasons that we will make precise in Section 7.4. (The Chi-squared is a special case of the Gamma and inherits exponential family membership through that route, but its degrees-of-freedom parameter $k$ is typically a fixed integer, not an estimated parameter.)
...
The natural parameter space is $\eta < 0$, identical to the Geometric. This makes sense: the Geometric is the Negative Binomial with $r = 1$, and setting $r = 1$ in $A(\eta) = -r\log(1 - e^\eta)$ recovers the Geometric log-partition function exactly.
</RemarkBlock>
...
<ProofBlock>
The Fisher-Neyman factorization theorem (which Sufficient Statistics (coming soon) will develop in full generality) states that $T(X)$ is sufficient for $\theta$ if and only if the joint density factors as
...
This is not a feature of specific distributions — it is a *structural consequence* of the exponential family form. Sufficient Statistics (coming soon) will develop the full theory, including the Rao-Blackwell theorem (sufficient statistics yield optimal estimators) and completeness (no redundancy in the sufficient statistic).
...
The sample mean is the MLE for the Normal mean — the single most foundational result in point estimation, and it requires exactly one line once we have the exponential family machinery. Maximum Likelihood (coming soon) will develop the general theory, including asymptotic properties and the multiparameter case.
</ExampleBlock>
...
| Geometric$(p)$ | $1$ | $\log(1-p)$ | $k$ | $-\log(1-e^\eta)$ | 1 |
| NegBin$(r,p)$ | $\binom{k-1}{r-1}$ | $\log(1-p)$ | $k$ | $-r\log(1-e^\eta)$ | 1 |
...
- Sufficient Statistics (coming soon) develops the full theory of data reduction, including the Rao-Blackwell theorem and completeness, building directly on the sufficient statistic $T(x)$ identified here
- Maximum Likelihood (coming soon) develops the asymptotic theory of MLE — consistency, asymptotic normality, efficiency — using the exponential family as the primary example class
