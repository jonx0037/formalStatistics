...
formalcalculusPrereqs:
  - topic: "sequences-limits"
    site: "formalcalculus"
    relationship: "The Monte-Carlo error in §31.2 hinges on the $B \\to \\infty$ limit of the empirical average $B^{-1}\\sum g(X^\\ast_{(b)})$ to its conditional expectation — sequence-limit machinery from formalcalculus. Bootstrap consistency asserts $\\sqrt n(\\hat\\theta^\\ast - \\hat\\theta) \\xrightarrow d$ to the limit law of $\\sqrt n(\\hat\\theta - \\theta)$, itself a sequence-limit of empirical measures."
  ...
  - topic: "approximation-theory"
    site: "formalcalculus"
    relationship: "The bootstrap approximates the true sampling distribution of an estimator by the bootstrap sampling distribution. The validity proof is an approximation statement: the bootstrap empirical process converges to the same Gaussian limit as the true empirical process, framed via the bracketing/covering apparatus of formalcalculus approximation-theory."
  - topic: "lp-spaces"
    site: "formalcalculus"
    relationship: "Bootstrap consistency is often stated as $L^2$ convergence of the bootstrap distribution to the true sampling distribution. Moment-matching arguments in second-order bootstrap validity proofs work in $L^2$ — the formalcalculus $L^p$ topic provides the rigorous normed-space substrate."
  - topic: "power-taylor-series"
    site: "formalcalculus"
    relationship: "The Edgeworth expansion of $\\sqrt n(\\hat\\theta_n - \\theta)$ in §31.5 is a Taylor-style series in the characteristic function with coefficients driven by cumulants. Bootstrap higher-order accuracy — $O(n^{-1})$ vs the CLT's $O(n^{-1/2})$ — follows from matching Edgeworth terms between bootstrap and true distributions."
formalmlConnections:
  ...
  - topic: "rank-tests"
    site: "formalml"
    relationship: "Bootstrap and permutation are both resampling-based approaches to inference but differ in their null-hypothesis treatment. Bootstrap resamples with replacement under the empirical distribution (no symmetry assumption); permutation resamples without replacement under exchangeability. §31.10 Rem 24 contrasts the two; rank-tests develops the permutation side."
connections:
