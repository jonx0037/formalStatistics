...
formalcalculusPrereqs:
  ...
  - topic: "derivative"
    site: "formalcalculus"
    relationship: "The Cramér–Rao lower bound differentiates under the integral sign — a maneuver justified by dominated-convergence conditions. The delta method for estimators of the form $g(\\hat\\theta)$ is a first-order Taylor expansion: $g(\\hat\\theta) \\approx g(\\theta) + g'(\\theta)(\\hat\\theta - \\theta)$."
formalmlConnections:
  - topic: "gradient-descent"
    site: "formalml"
    relationship: "The mini-batch gradient is a biased estimator of the full gradient (if the batch is a strict subset); its variance is $\\sigma^2/B$ for batch size $B$. The learning-rate schedule navigates the bias-variance tradeoff in optimization."
  - topic: "causal-inference-methods"
    site: "formalml"
    relationship: "AIPW (§6), TMLE (§7), and DML (§8) are M-estimators / Z-estimators with augmented score functions; their asymptotic normality follows from the Stefanski–Boos (2002) M-estimator framework developed in this topic. The §4.3 sandwich-variance formula is the standard point-estimation derivation applied to causal estimators."
  - topic: "semiparametric-inference"
    site: "formalml"
    relationship: "M-estimator / Z-estimator asymptotics — consistency, asymptotic normality, and sandwich-variance formula derived in §§6–9 of point-estimation — are the parametric substrate this topic builds the semiparametric framework on top of. The §10 sandwich-SE derivation is a direct lift."
connections:
...
2. **Sample median:** $\hat{\mu}_2 = \operatorname{median}(X_1, \ldots, X_n)$, the middle order statistic — whose asymptotics are developed in [Topic 29 §29.6](/topics/order-statistics-and-quantiles#section-29-6).
...
When $\sigma$ is unknown — the usual case — we plug in the sample standard deviation $S = \sqrt{S^2}$ (from Example 3) to get the **estimated** standard error $\widehat{\operatorname{SE}}(\bar{X}_n) = S/\sqrt{n}$. The distinction matters: confidence intervals built from $S$ use the Student's $t$ distribution, not the standard Normal, because $S$ introduces additional uncertainty. That $t$-correction is Topic 11's Example 7, and the machinery of [confidence intervals](/topics/confidence-intervals-and-duality) will develop it systematically.
</ExampleBlock>
...
When regularity holds, the CRLB is the ceiling: no unbiased estimator can beat $1/(nI(\theta))$. When regularity fails, different tools — the $L_1$ theory, order-statistics asymptotics ([Topic 29 §29.6](/topics/order-statistics-and-quantiles#section-29-6)), the Hájek-Le Cam framework — take over. For standard statistical modeling, regularity holds and the CRLB is the right benchmark.
</RemarkBlock>
...
**Pointer.** The full proof uses Stein's identity: for $Z \sim \mathcal{N}(0, 1)$ and smooth $g$, $\mathbb{E}[Z\, g(Z)] = \mathbb{E}[g'(Z)]$. Applied coordinate-wise to the squared error of $\hat{\theta}_{JS}$, it reduces the MSE calculation to an integral involving $(d-2)/\|X\|^2$. See Lehmann & Casella (1998), Chapter 5, for the detailed argument; [Topic 28 §28.5](/topics/hierarchical-bayes-and-partial-pooling#section-28-5) now also gives the full Stein-identity proof in the hierarchical-Bayes context. $\quad\blacksquare$
</ProofBlock>
...
Beyond Track 4, every downstream topic in formalStatistics uses this vocabulary. **[Hypothesis Testing](/topics/hypothesis-testing)** builds test statistics that are estimators; their null and alternative distributions are the consistency and asymptotic-normality statements we have just proved. The z-test applies the CLT to a standardized $\bar X$; the t-test uses Basu's theorem to give $\sqrt n(\bar X - \mu_0)/S$ a clean $t_{n-1}$ distribution; the Wald, score, and likelihood-ratio tests are all $\chi^2$-asymptotic consequences of Thm 14.3's asymptotic-normality result. **[Confidence Intervals](/topics/confidence-intervals-and-duality)** invert asymptotic-normality: a $1 - \alpha$ CI is an interval of the form $\hat{\theta}_n \pm z_{\alpha/2}\cdot \widehat{\operatorname{SE}}$. **[Linear Regression](/topics/linear-regression)** applies Gauss–Markov efficiency to OLS, which is the CRLB for Normal-error linear models; regularization and shrinkage apply Stein's phenomenon. **[The Bootstrap](/topics/bootstrap)** gives empirical analogs of the sampling distribution when closed-form asymptotics are unavailable. Across [formalML](https://formalml.com), the bias-variance tradeoff organizes regularization, ensembling, transfer learning, and hyperparameter tuning — the daily tools of every machine-learning practitioner.
