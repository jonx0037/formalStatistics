...
This is the strong law of large numbers from [Topic 10](/topics/law-of-large-numbers), restated in estimator language. The sample mean is the prototypical consistent estimator, and the LLN is the engine: consistency of the sample mean is *literally* the LLN under a different name. Every estimator that can be written as (or derived from) a sample mean inherits consistency from the LLN — this is the template that Topic 14 will use to prove the MLE is consistent, and Topic 15 to handle method-of-moments estimators.
