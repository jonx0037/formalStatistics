...
formalcalculusPrereqs:
  ...
  - topic: "gradient"
    site: "formalcalculus"
    relationship: "Stein's identity in §28.5 is integration by parts applied coordinatewise to $g(X) = X/\\|X\\|^2$; the full proof relies on the multivariate gradient of this gradient field. Empirical-Bayes estimation of hyperparameters uses gradient-based optimization of the marginal likelihood, and full Bayes via HMC needs gradients of the hierarchical log-posterior."
  - topic: "multiple-integrals"
    site: "formalcalculus"
    relationship: "The hierarchical-model marginal likelihood $p(x \\mid \\phi) = \\int p(x \\mid \\theta)\\,p(\\theta \\mid \\phi)\\,d\\theta$ integrates over the individual-level parameter $\\theta$. Full Bayes integrates further over $\\phi$; these nested integrals define the partial-pooling structure §28.6 develops."
formalmlConnections:
  - topic: "probabilistic-programming"
    site: "formalml"
    relationship: "§5's eight-schools example IS a hierarchical model with partial pooling. This topic develops the partial-pooling theory — when the hierarchical structure improves estimates over no-pooling and complete-pooling alternatives — that probabilistic-programming applies as an instance."
