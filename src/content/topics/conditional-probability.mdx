...
formalcalculusPrereqs:
  ...
  - topic: "radon-nikodym"
    site: "formalcalculus"
    relationship: "The Radon–Nikodym theorem is exactly the existence proof for conditional expectation $\\mathbb E[X \\mid \\mathcal G]$: given a $\\sigma$-finite measure $P$ restricted to $\\mathcal G$ and the measure $\\nu(A) = \\int_A X\\,dP$ on $\\mathcal G$, the RN derivative $d\\nu/dP|_{\\mathcal G}$ is $\\mathbb E[X \\mid \\mathcal G]$. Conditional probability is its special case for indicator integrands."
  - topic: "sigma-algebras"
    site: "formalcalculus"
    relationship: "Conditional expectation $\\mathbb E[X \\mid \\mathcal G]$ is defined as the unique (a.s.) $\\mathcal G$-measurable random variable satisfying $\\mathbb E[X \\cdot \\mathbb 1_A] = \\mathbb E[\\mathbb E[X \\mid \\mathcal G] \\cdot \\mathbb 1_A]$ for all $A \\in \\mathcal G$ — a $\\sigma$-algebra-level construction. The Radon–Nikodym theorem guarantees its existence."
formalmlConnections:
...
The full development of Bayesian inference — conjugate priors, MCMC, variational methods — lives in [Bayesian Foundations (Topic 25)](/topics/bayesian-foundations-and-prior-selection) within formalStatistics and <ExternalLink href="https://formalml.com/topics/bayesian-inference" site="formalml" topic="Bayesian Inference" /> on formalML.
...
**[Random Variables & Distribution Functions](/topics/random-variables)** extends these ideas from events to numbers. A random variable $X : \Omega \to \mathbb{R}$ is a measurable function that translates the abstract probability space into numerical statements. Conditional distributions $P(X \leq x \mid Y = y)$, conditional expectation $E[X \mid Y]$, and the law of total expectation $E[X] = E[E[X \mid Y]]$ all build directly on the conditional probability framework developed here. See [Expectation, Variance & Moments](/topics/expectation-moments) for the full treatment of conditional expectation and the tower property.
