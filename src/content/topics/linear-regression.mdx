...
formalmlConnections:
  ...
  - topic: "high-dimensional-regression"
    site: "formalml"
    relationship: "When $p > n$, $\\mathbf{X}^\\top \\mathbf{X}$ is singular and OLS fails. Topic 23 (ridge, lasso) is the Track-6 answer; the high-dimensional theory (double descent, minimax rates) lives at formalml. §21.10 Rem 28 states the forward pointer."
connections:
...
When $p > n$, OLS fails and penalized methods (Topic 23) become essential. The theory of **high-dimensional regression** — sparsity rates, minimax bounds, double descent — lives at <ExternalLink site="formalml" topic="High-dimensional regression" href="https://formalml.com/topics/high-dimensional-regression" />. The modern regime where $p \gg n$ with kernel-like implicit regularization (neural networks, random features) is an active research area that Track 8 and formalml.com's theory topics touch.
</RemarkBlock>
