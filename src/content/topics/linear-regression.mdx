...
formalcalculusPrereqs:
  ...
  - topic: "banach-spaces"
    site: "formalcalculus"
    relationship: "The projection of $\\mathbf{y}$ onto $\\mathrm{col}(\\mathbf{X})$ — the OLS fit — is a bounded linear operator on $\\mathbb{R}^n$ (a finite-dimensional Banach/Hilbert space). The operator-theoretic view generalizes to infinite-dimensional regression problems in nonparametric and functional-data contexts."
  - topic: "hilbert-spaces"
    site: "formalcalculus"
    relationship: "OLS is the orthogonal projection of $\\mathbf{y}$ onto $\\mathrm{col}(\\mathbf{X})$ in $\\mathbb{R}^n$ as Hilbert space. The Pythagorean decomposition $\\|\\mathbf{y}\\|^2 = \\|\\hat{\\mathbf{y}}\\|^2 + \\|\\mathbf{y} - \\hat{\\mathbf{y}}\\|^2$ underlies the ANOVA identity $\\mathrm{SST} = \\mathrm{SSR} + \\mathrm{SSE}$. The normal equations $\\mathbf{X}^\\top \\mathbf{X}\\hat{\\boldsymbol\\beta} = \\mathbf{X}^\\top \\mathbf{y}$ express orthogonality of residuals to the column space."
formalmlConnections:
  ...
  - topic: "mixed-effects"
    site: "formalml"
    relationship: "§21.6 Gauss–Markov (OLS as BLUE under homoscedastic errors) is the result mixed-effects §2.4's GLS-as-BLUE / Aitken whitening specializes: the whitening map $\\boldsymbol\\Omega^{-1/2}$ reduces GLS on the correlated-error system to OLS on a whitened system, and BLUE-preservation under whitening follows directly from applying §21.6 to the transformed design. Mixed-effects §2's GLS proof is therefore §21.6 plus a change of basis."
  - topic: "double-descent"
    site: "formalml"
    relationship: "The OLS substrate this topic builds — normal equations, projection geometry, bias-variance decomposition, condition-number diagnostics — is what double-descent's classical $\\sigma^2(k+1)/n$ U-curve specializes. The overparameterized regime extends the same decomposition with bias and variance components governed by Marchenko–Pastur integrals."
  - topic: "kernel-regression"
    site: "formalml"
    relationship: "The kernel-regression §2.3 reframing of Nadaraya–Watson as degree-zero local polynomial regression draws directly on this topic's weighted-least-squares theory. §8.2's local-linear extension is locally-weighted OLS with intercept plus slope; its WLS solution generalizes the single-coefficient form via kernel-weighted normal equations."
  - topic: "local-regression"
    site: "formalml"
    relationship: "Locally-weighted least squares with a degree-$p$ polynomial design matrix is the central machinery of local-regression. The $(X^\\top W X)^{-1} X^\\top W y$ normal equations are the linear-regression formulas applied at every evaluation point, and the §3 equivalent-kernel formulation is the projection-onto-column-space picture from this topic read in kernel-smoothing coordinates."
  - topic: "quantile-regression"
    site: "formalml"
    relationship: "Quantile regression's Theorem 3 asymptotic normality $\\sqrt n(\\hat\\beta(\\tau) - \\beta(\\tau)) \\Rightarrow N(0, \\tau(1-\\tau)\\, D^{-1}\\Omega D^{-1})$ is the QR replacement of OLS's $\\sqrt n(\\hat\\beta_{\\mathrm{OLS}} - \\beta) \\Rightarrow N(0, \\sigma^2(X^\\top X)^{-1})$. The Bernoulli factor $\\tau(1-\\tau)$ plays the role of $\\sigma^2$; the density-weighted Gram matrix sandwich-replaces $(X^\\top X)^{-1}$."
  - topic: "sparse-bayesian-priors"
    site: "formalml"
    relationship: "Sparse Bayesian regression specializes the linear-regression machinery of this topic to the $p > n$ regime where ordinary OLS is rank-deficient. The §10 diabetes benchmark builds on the linear-regression substrate established here — design-matrix algebra, noise-variance estimation, prior-on-coefficients framing — all inherited directly."
  - topic: "structural-risk-minimization"
    site: "formalml"
    relationship: "Parametric substrate for SRM's polynomial-regression worked example. The bias-variance decomposition and Gauss–Markov variance bound are used by name — the U-shape of MSE-versus-capacity is read off the parametric variance term $\\sigma^2(k+1)/n$ from this topic's least-squares analysis."
connections:
...
Adjusted $R^2$ is a limited criterion. [**AIC** (Akaike)](/topics/model-selection-and-information-criteria#section-24-3) penalizes by $2k$; [**BIC** (Schwarz)](/topics/model-selection-and-information-criteria#section-24-4) by $k\log n$; [**cross-validation**](/topics/model-selection-and-information-criteria#section-24-5) estimates out-of-sample predictive performance directly, and Stone's 1977 theorem identifies LOO-CV with AIC asymptotically. [Mallows' $C_p$](/topics/model-selection-and-information-criteria#section-24-2) is the Gaussian-linear special case. The three criteria diverge in the large-$p$ regime: BIC is selection-consistent when the true model is in the candidate set, AIC and CV are minimax-rate optimal for prediction, and [Yang 2005](/topics/model-selection-and-information-criteria#section-24-6) proves the two properties are formally incompatible. [Topic 24](/topics/model-selection-and-information-criteria) develops the full theory.
</RemarkBlock>

<RemarkBlock number={25} title="Track 7 — Bayesian linear regression">
A conjugate Normal-inverse-gamma prior on $(\boldsymbol\beta, \sigma^2)$ yields a closed-form Normal-inverse-gamma posterior, with the posterior mean an explicit ridge-regression estimate (the prior precision plays the role of $\lambda$). The Bayesian CI is the **credible interval** — a subset of parameter space with posterior mass $1 - \alpha$ — which agrees with the Wald-t CI only under flat priors and as $n \to \infty$. The conditional-MVN calculation of [Topic 8 §8.4](/topics/multivariate-distributions) is the mechanical engine. [Topic 25 §25.5](/topics/bayesian-foundations-and-prior-selection#section-25-5) covers the scalar Normal–Normal-Inverse-Gamma case; non-conjugate priors are handled via [MCMC](/topics/bayesian-computation-and-mcmc) ([Topic 26](/topics/bayesian-computation-and-mcmc)), with the regression extension developed in Topics 27–28.
</RemarkBlock>

<RemarkBlock number={26} title="Track 8 — Nonparametric regression">
When the linearity assumption itself is too restrictive, **nonparametric regression** estimates $E[Y \mid \mathbf{x}]$ without assuming a parametric form. Kernel regression (Nadaraya–Watson), local polynomial regression, and smoothing splines all replace the parametric $\mathbf{x}^\top \boldsymbol\beta$ with a data-driven smooth function. The bias-variance trade-off is controlled by a bandwidth or smoothing parameter; Track 8's [Topic 30 (Kernel Density Estimation)](/topics/kernel-density-estimation) treats the density-estimation theory, and the covariate-conditional (regression) extension is developed at formalml.
</RemarkBlock>
