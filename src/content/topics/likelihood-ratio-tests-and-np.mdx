...
formalcalculusPrereqs:
  ...
  - topic: "derivative"
    site: "formalcalculus"
    relationship: "The score test statistic $U(\\theta_0)^2/I(\\theta_0)$ and the Wald statistic $(\\hat\\theta - \\theta_0)^2 I(\\hat\\theta)$ are both built from first and second derivatives of the log-likelihood. The asymptotic chi-squared distribution follows from a Taylor expansion around $\\theta_0$."
  - topic: "radon-nikodym"
    site: "formalcalculus"
    relationship: "The likelihood ratio $\\Lambda(x) = f_1(x)/f_0(x) = dP_1/dP_0$ is the Radon–Nikodym derivative of the alternative w.r.t. the null. The Neyman–Pearson lemma — that the LRT is the most powerful test at any level — is proven using monotonicity properties of RN derivatives."
formalmlConnections:
  - topic: "rank-tests"
    site: "formalml"
    relationship: "The Neyman–Pearson optimal test under correctly-specified parametric models — i.e., the $t$-test under normality — is the parametric foil against which rank-tests §5's ARE compares Wilcoxon. The ARE framework asks: how much power do we lose by giving up the parametric model?"
connections:
