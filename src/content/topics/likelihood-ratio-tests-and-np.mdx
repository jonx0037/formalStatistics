...
formalmlConnections:
  ...
  - topic: "model-comparison"
    site: "formalml"
    relationship: "Wilks' theorem is the frequentist backbone of the likelihood-ratio test for nested models — the answer to 'did the extra parameters help?' The Bayesian analog is the Bayes factor ([Topic 27 §27.4](/topics/bayesian-model-comparison-and-bma#section-27-4)); the ML-native analog is cross-validated held-out likelihood."
connections:
...
- **Track 6 (Regression & Linear Models).** The Wald/Score/LRT trio of §18.7 reappears as the three standard GLM inference procedures — deviance tests are LRT, z-tests on coefficients are Wald, forward-selection score tests are Rao. [Linear regression's F-test](/topics/linear-regression) is exactly Wilks' theorem specialized to nested linear models — §21.8 Thm 9 delivers the exact $F_{k, n-p-1}$ finite-sample distribution as the sharpening of Topic 18 Thm 4.

- **Track 7 (Bayesian Statistics).** Bayes factors (the posterior-odds analog of the LRT) are the Bayesian counterpart to §18.5–§18.7. Frequentist/Bayesian testing duality is non-trivial — Lindley's paradox ([Topic 27 §27.5](/topics/bayesian-model-comparison-and-bma#section-27-5)) is the first sharp disagreement.
