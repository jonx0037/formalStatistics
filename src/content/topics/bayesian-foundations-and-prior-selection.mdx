...
formalcalculusPrereqs:
  ...
  - topic: "multiple-integrals"
    site: "formalcalculus"
    relationship: "The marginal likelihood $m(\\mathbf{y}) = \\int L(\\theta)\\pi(\\theta)\\,d\\theta$ and posterior predictive $p(\\tilde y \\mid \\mathbf{y}) = \\int p(\\tilde y \\mid \\theta)p(\\theta \\mid \\mathbf{y})\\,d\\theta$ are integrals against the prior / posterior density over the full parameter space. In $d$ dimensions Fubini justifies iterated computation; §25.4 Proof 1 closes the conjugate-family case by identifying the integrand's kernel with a known density family."
  - topic: "radon-nikodym"
    site: "formalcalculus"
