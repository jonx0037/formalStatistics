...
formalcalculusPrereqs:
  ...
  - topic: "hilbert-spaces"
    site: "formalcalculus"
    relationship: "Gaussian-process priors over functions are specified by a mean and covariance kernel — a Hilbert-space structure. The Cameron–Martin space associated with a GP prior is the RKHS of the kernel; posterior contractions happen in this Hilbert-space geometry, which is what the formalcalculus hilbert-spaces topic underwrites."
  - topic: "improper-integrals"
    site: "formalcalculus"
    relationship: "Conjugate priors (Beta–Binomial, Gamma–Poisson, Normal–Normal) are constructed so the posterior normalizer reduces to a ratio of Gamma or Beta functions. Improper priors (Jeffreys, flat) require the improper-integral machinery to verify posterior propriety."
  - topic: "multiple-integrals"
    site: "formalcalculus"
    relationship: "The posterior $p(\\theta \\mid x) = p(x \\mid \\theta)\\pi(\\theta)/\\int p(x \\mid \\theta)\\pi(\\theta)\\,d\\theta$ has a normalizing integral over the full parameter space. In $d$ dimensions this is a $d$-dimensional integral; Fubini justifies iterated computation when possible."
  - topic: "radon-nikodym"
    site: "formalcalculus"
    relationship: "Posterior densities $p(\\theta \\mid x)$ are Radon–Nikodym derivatives of the posterior measure w.r.t. a dominating measure (Lebesgue on $\\mathbb R^d$, counting on a countable set). Likelihoods $L(\\theta; x)$ are RN derivatives of the data measure across different $\\theta$; Bayes' rule is a change-of-measure identity."
formalmlConnections:
  ...
  - topic: "mixed-effects"
    site: "formalml"
    relationship: "§25.3 Thm 1 (posterior $\\propto$ prior $\\times$ likelihood) is the framework mixed-effects §5's PyMC fit operates inside; §25.5–§25.7's prior-selection theory — conjugate, weakly-informative, and Jeffreys — supplies the menu §5.1 chooses from on variance-component hyperparameters. §25.7's improper-prior theory contains the flat-improper case mixed-effects §4.2 marginalizes over the fixed effects to recover the REML profile likelihood."
    url: "https://formalml.com/topics/mixed-effects"
  - topic: "sparse-bayesian-priors"
    site: "formalml"
    relationship: "Sparse priors are downstream of the prior-elicitation framework developed in this topic. §6's $m_0$ hyperparameter (regularized horseshoe's prior expected number of effective coefficients) and §8's $(a, b, \\xi)$ hyperprior on R2-D2 are concrete elicitations of the §25.5 weakly-informative prior class."
  - topic: "stacking-and-predictive-ensembles"
    site: "formalml"
    relationship: "Develops prior elicitation; §4.1's four candidate priors (weakly informative Gaussians, half-Normal scales, inverse-gamma length scales) build directly on the §25.5–§25.7 elicitation framework, and §6.1's sensitivity-analysis use case is the topic-level application."
  - topic: "variational-bayes-for-model-selection"
    site: "formalml"
    relationship: "VBMS is downstream of the prior–likelihood–posterior decomposition and depends critically on hyperprior choice. §4's Bayesian-GMM ARD result is sensitive to the Dirichlet concentration $\\alpha$; §11's computational notes return to prior-sensitivity diagnostics."
connections:
...
Proof is a direct application of [Topic 4 Thm 4](/topics/conditional-probability) to the joint density $p(\theta, \mathbf{y}) = \pi(\theta) L(\theta; \mathbf{y})$; the computation is exactly the event version with $P$ replaced by density integrals.
</TheoremBlock>
