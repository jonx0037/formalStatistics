...
formalcalculusPrereqs:
  ...
  - topic: "sigma-algebras"
    site: "formalcalculus"
    relationship: "A probability space $(\\Omega, \\mathcal F, P)$ has $\\mathcal F$ a $\\sigma$-algebra and $P$ a probability measure on $\\mathcal F$. Kolmogorov's axioms are measure-theoretic at base; the Carathéodory extension theorem from the formalcalculus $\\sigma$-algebras topic is the bedrock that makes every construction here rigorous."
formalmlConnections:
  - topic: "vc-dimension"
    site: "formalml"
    relationship: "VC-dimension theory's $(\\varepsilon, \\delta)$-PAC framework is built on the same $\\sigma$-algebra-and-probability-measure substrate as classical probability. The §1 sample-space rigor here is what makes PAC's 'with probability at least $1 - \\delta$' statements well-defined as events on a product probability space."
...
The next topic — [**Conditional Probability & Independence**](/topics/conditional-probability) — builds on this foundation to answer: "how does new information change probabilities?" We'll derive Bayes' theorem, formalize independence, and see why conditional independence is the structural assumption that makes probabilistic ML tractable.
