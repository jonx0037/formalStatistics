...
$$
\sqrt{n}\!\left(\hat{\lambda}_n - \lambda\right) \;\xrightarrow{d}\; \mathcal{N}\!\left(0, \lambda^{-4} \cdot \lambda^{-2}\right) \;=\; \mathcal{N}(0, \lambda^2 \cdot \text{something wrong}).
$$

Wait — be careful. With $g(x) = 1/x$ at $x = 1/\lambda$, $g'(1/\lambda) = -\lambda^2$. So $[g'(\theta)]^2 \sigma^2 = \lambda^4 \cdot \lambda^{-2} = \lambda^2$, giving
...
**What comes next.** [Large Deviations & Tail Bounds](/topics/large-deviations) (coming soon) complements the CLT: where the CLT gives the shape of fluctuations at scale $1/\sqrt{n}$, large-deviations theory gives exponential rates at all scales, including the tails where the CLT is weakest. Together they give a complete picture of how sample means behave. Beyond Track 3, the CLT is the foundation for confidence intervals (coming soon), hypothesis testing (coming soon), the bootstrap (coming soon), and every asymptotic result in the statistical-inference tracks.
