...
formalcalculusPrereqs:
  ...
  - topic: "fourier-series"
    site: "formalcalculus"
    relationship: "Characteristic functions $\\varphi_X(t) = \\mathbb{E}[e^{itX}]$ are Fourier transforms of densities; Lévy's continuity theorem (convergence of characteristic functions implies convergence in distribution) is an application of Fourier inversion. The CLT is cleanest to prove on the Fourier side via Taylor expansion of $\\log\\varphi$."
  - topic: "mean-value-taylor"
    site: "formalcalculus"
    relationship: "The characteristic-function proof of the CLT expands $\\log\\varphi_X(t)$ to second order in $t$ near 0: $\\log\\varphi_X(t) = i\\mu t - \\sigma^2 t^2/2 + o(t^2)$. Summing and rescaling gives the Gaussian characteristic function in the limit — pure Taylor expansion with an MVT-controlled remainder."
  - topic: "power-taylor-series"
    site: "formalcalculus"
    relationship: "The characteristic-function proof of the CLT expands $\\log\\varphi_{X_i}(t/\\sqrt n)$ as a Taylor series in $t$. The dominant terms give the Gaussian characteristic function $e^{-\\sigma^2 t^2/2}$; the remainder terms vanish as $n \\to \\infty$ at rate set by the Berry–Esseen third-moment bound."
formalmlConnections:
  - topic: "gradient-descent"
    site: "formalml"
    relationship: "The mini-batch gradient is an average of $B$ iid gradient samples; by the CLT, its fluctuations around the true gradient are Gaussian at scale $\\sigma/\\sqrt B$, which motivates the learning-rate schedule $\\eta \\propto 1/\\sqrt t$ and explains why larger batches reduce noise with diminishing returns."
  - topic: "generalization-bounds"
    site: "formalml"
    relationship: "Rademacher complexity bounds use symmetrization and CLT-type reasoning. The empirical Rademacher complexity converges to its expectation at rate $1/\\sqrt n$, giving the $O(1/\\sqrt n)$ generalization bound for bounded loss functions."
  - topic: "bayesian-neural-networks"
    site: "formalml"
    relationship: "The Bernstein–von Mises theorem (the Bayesian CLT) proves that the posterior $\\sqrt n(\\theta - \\theta_0) \\mid X_1, \\ldots, X_n$ converges in total variation to $N(0, I(\\theta_0)^{-1})$, where $I(\\theta_0)$ is the Fisher information — why Bayesian credible intervals and frequentist confidence intervals agree asymptotically."
  - topic: "rank-tests"
    site: "formalml"
    relationship: "Required for the asymptotic null distributions throughout rank-tests: the normal approximation to $W^+$ in §1, $U$ in §3, and the multivariate finite-population CLT (Hájek 1960) that powers §4's chi-squared limit. §5's Pitman ARE proof leans on the joint CLT under contiguous alternatives."
  - topic: "stochastic-gradient-mcmc"
    site: "formalml"
    relationship: "§12's head-to-head benchmark uses Bayesian logistic regression with asymptotically-Gaussian posterior by Bernstein–von Mises; CLT is the underlying mechanism for that asymptotic Gaussianity."
connections:
