...
formalcalculusPrereqs:
  ...
  - topic: "jacobian"
    site: "formalcalculus"
    relationship: "The multivariate change-of-variables formula for densities requires the Jacobian of the transformation. Polar-to-Cartesian, spherical coordinates, and linear transformations of the multivariate Normal all invoke Jacobian determinants — the formalcalculus jacobian topic provides the rigorous foundation."
  - topic: "multiple-integrals"
    site: "formalcalculus"
    relationship: "A joint density $f(x_1, \\ldots, x_n)$ integrates to 1 over $\\mathbb R^n$ — a multivariate integral. Marginalizing $f_i(x_i) = \\int f(x_1, \\ldots, x_n)\\,dx_{-i}$ is an iterated integral (justified by Fubini); the multivariate-Normal normalizer $(2\\pi)^{d/2}\\sqrt{\\det\\Sigma}$ comes from the multivariate Gaussian integral."
formalmlConnections:
  ...
  - topic: "mixed-effects"
    site: "formalml"
    relationship: "§4 Thm 3's conditional-MVN formula $\\boldsymbol\\mu_{1|2} = \\boldsymbol\\mu_1 + \\boldsymbol\\Sigma_{12}\\boldsymbol\\Sigma_{22}^{-1}(\\mathbf{x}_2 - \\boldsymbol\\mu_2)$ (the Schur-complement reading) is the pure-probability identity mixed-effects §3 specializes to derive BLUP as the posterior mean $\\mathrm{Cov}(\\mathbf{u}, \\mathbf{y})\\,\\mathrm{Cov}(\\mathbf{y})^{-1}(\\mathbf{y} - \\mathbb{E}[\\mathbf{y}])$. Mixed-effects §2's marginal-distribution theorem reduces to the §3 affine-transform result here — linear maps preserve MVN."
  - topic: "clustering"
    site: "formalml"
    relationship: "The multivariate-Gaussian-kernel substrate (clustering §6.1) and the Mahalanobis-distance reading of GMM responsibilities (clustering §10.2) require the multivariate-normal machinery — covariance matrices, density form, marginal/conditional rules — that this topic develops."
  - topic: "statistical-depth"
    site: "formalml"
    relationship: "Statistical-depth §3.1's Mahalanobis-depth construction and §5.1's DD-classifier optimality theorem both rest on the elliptical-symmetry framework articulated here. The depth-of-a-density connection — that depth functions reach their maximum at the density mode for elliptical distributions — is a direct corollary of §7's geometric picture."
connections:
...
The full development of Wishart-based inference, including the Normal-Inverse-Wishart conjugate family for joint inference on $(\boldsymbol{\mu}, \boldsymbol{\Sigma})$, belongs to [Bayesian Computation](/topics/bayesian-computation-and-mcmc).
</RemarkBlock>
