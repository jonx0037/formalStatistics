...
We begin with the general definitions. These are direct extensions of the bivariate definitions from [Topic 3](/topics/random-variables), but stated for an arbitrary dimension $p$. The key prerequisite is comfort with iterated integrals over $\mathbb{R}^p$ — see <ExternalLink href="https://formalcalculus.com/topics/multiple-integrals" site="formalcalculus">Multiple Integrals & Fubini's Theorem</ExternalLink> for Fubini's theorem and the mechanics of integrating over subsets of $\mathbb{R}^p$.
