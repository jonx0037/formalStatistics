...
The z-test, t-test, and $\chi^2$-test all rely on special distributional assumptions (Normality and known families) to give exact null distributions. For general parametric models, three asymptotic tests built directly from the likelihood are the workhorses. They agree to first order under $H_0$ and diverge in finite-sample power — a divergence that Topic 18 treats in full.
...
- **Likelihood-Ratio Tests & Neyman-Pearson (coming soon, Topic 18)** — the optimality theory. Neyman-Pearson's lemma: the LRT is UMP for simple-vs-simple. Uniformly most powerful tests, monotone likelihood ratio families, Wilks' theorem proved in full, the three asymptotic tests' finite-sample power compared.
